...
const packages = [
  { name: "Trial Ad", price: "$99", intro: "Three ads for your first product test.", ads: "3", longer: "0", items: ["Three finished ads with distinct openings and payoffs", "Each up to 30 seconds, vertical 9:16", "One product and one batch revision round"], checkout: "https://whop.com/checkout/plan_6OrvvrFEUnYa1", featured: true },
  { name: "Test Sprint", price: "$299", intro: "Short ads plus a deeper product explanation.", ads: "5", longer: "1", items: ["Five ads up to 30 seconds across two directions", "One 60–90 second product explainer", "One product and one batch revision round"], checkout: "https://whop.com/checkout/plan_ospIIPGODzxd1" },
  { name: "Growth Sprint", price: "$699", intro: "More directions and room to explain the product.", ads: "8", longer: "2", items: ["Eight ads up to 30 seconds across four directions", "Two 60–90 second product explainers", "One product and one batch revision round"], checkout: "https://whop.com/checkout/plan_fSMAw8tgMTaBT" },
  { name: "Scale Batch", price: "$1,499", intro: "A creative queue with short and long ads.", ads: "12", longer: "4", items: ["Twelve ads up to 30 seconds across six directions", "Four 60–90 second explainers and a testing sequence", "One product and one batch revision round"], checkout: "https://whop.com/checkout/plan_V7DWvqie2zGrG" },
];
const questions = [
    ...
    <main id="main">
      ...
      <section id="work" className="section-space work-section">
        <div className="page-width">
          ...
          <div className="section-heading"><h2>Different products.<br/><em>Distinct stories.</em></h2><p>Seven videos across beauty, pets, travel, personal care and cars. Three are our spec ads; four are supplied creative references.</p></div>
      ...
      <section id="pricing" className="section-space pricing-section"><div className="page-width"><div className="section-topline"><span className="eyebrow">04 / Pick your pace</span><span className="tiny-note">ONE PRODUCT. ONE-TIME PAYMENT. DEFINED CREATIVE.</span></div><div className="section-heading"><h2>Start small.<br/><em>Build on what works.</em></h2><p>Start with three finished ads for $99. Larger batches add distinct creative directions and 60–90 second explainers. Shared approved footage helps make the batch efficient.</p></div>
        <div className="pricing-grid">{packages.map(pack=><article key={pack.name} className={`price-card ${pack.featured ? "price-featured" : ""}`}><div className="price-top"><h3>{pack.name}</h3>{pack.featured && <span>FIRST TEST</span>}</div><p className="price-intro">{pack.intro}</p><div className="price-amount">{pack.price}<span>USD / project</span></div><div className="price-stats"><div><strong>{pack.ads}</strong><span>ads up to 30s</span></div><div><strong>{pack.longer}</strong><span>60–90s explainers</span></div></div><ul>{pack.items.map(item=><li key={item}><Check size={15}/><span>{item}</span></li>)}</ul><a href={pack.checkout} onClick={()=>choosePackage(pack.name)} className={`ss-button ${pack.featured ? "ss-button-orange" : "ss-button-outline"}`}><span>{`Pay for ${pack.name}`}</span><ArrowUpRight size={17}/></a></article>)}</div>
